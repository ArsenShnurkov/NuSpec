...
namespace NuSpec.RazorHosting.Core
{
	...
	public class RazorEngine<TBaseTemplateType> : MarshalByRefObject where TBaseTemplateType : RazorTemplateBase
	{
		/// <summary>
		...
		/// </summary>
		/// <param name="templateSourceReader"></param>
			...
			string outputFile)
		{
			...
			{
				using(var writer = new StreamWriter(outputFile, false, Configuration.OutputEncoding, Configuration.StreamBufferSize))
				{
					result = RenderTemplate(sourceCodeReader, referencedAssemblies, context, writer);
				}
			}
			return result!=null;
		}
